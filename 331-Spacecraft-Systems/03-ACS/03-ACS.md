...
This lecture develops the spacecraft **Attitude Control System (ACS)**
from its basic purpose and terminology through disturbance analysis,
mission pointing geometry, attitude-determination sensors, control
actuators/effectors, architecture selection, sizing, rotational
dynamics, simulation, and flight anomaly case studies.

The lecture begins with the function of ACS: providing **attitude
control, knowledge, and stability** to support spacecraft pointing. It
introduces the relationship between commanded attitude, measured
attitude, attitude error, sensors, controllers, actuators, and external
disturbances, then uses **OSIRIS-REx** to connect ACS hardware and
pointing requirements to a real mission.

The main technical flow is:

1.  Understand ACS functions, terminology, and closed-loop control.
2.  Identify the disturbance torques acting on the spacecraft.
3.  Understand mission geometry and pointing constraints.
4.  Select attitude-determination sensors.
5.  Select control actuators/effectors.
6.  Develop physical and logical ACS architectures.
7.  Address redundancy and component sizing.
8.  Model spacecraft rotational kinematics and dynamics.
9.  Build and validate end-to-end attitude-control simulations.
10. Use flight case studies to understand how modeling, hardware,
    software, coordinate frames, testing, and fault response interact.

Major disturbance sources include **aerodynamic, gravity-gradient,
magnetic, solar-radiation, mass-expulsion, launch tip-off, and internal
torques**. Attitude-determination hardware includes **sun sensors, Earth
sensors, magnetometers, star trackers, gyroscopes/IMUs, and GPS
receivers**. Control effectors include **reaction wheels, momentum
wheels, control moment gyros (CMGs), magnetic torque rods, and
reaction-control thrusters**.

The lecture also develops the rotational physics behind ACS, including
**moment of inertia, angular acceleration, angular momentum, torque,
inertia tensors, Euler's moment equations, Euler angles, and rotation
matrices**.
...
-   [1. ACS Introduction](#1-acs-introduction)
-   [2. OSIRIS-REx Mission Example](#2-osiris-rex-mission-example)
...
-   [7. ACS Architecture, Sizing, and
    Design](#7-acs-architecture-sizing-and-design)
-   [8. Questions and Additional ACS
    Topics](#8-questions-and-additional-acs-topics)
...
-   GRAIL Example
    ...
    -   Provide attitude control, knowledge, and
...
For example, imagine the boresight of a telescope is represented by the
vectors shown below:

-   Attitude: Orientation of a defined reference system attached to the
    spacecraft body

y

Attitude (actual) Desired attitude (eg, payload requirements) Attitude
knowledge (based on sensors)

-   Determination: Knowledge within a specified tolerance (realtime or
    post- facto)

x

-   Control: Maintenance of specified attitude within a given tolerance

Attitude Errors

z

-   Pointing Error: Low frequency component of attitude misalignment
...
Jitter/Stability (typically uncorrected)

------------------------------------------------------------------------

# 2. OSIRIS-REx Mission Example
...
formed and how life began, as well as improve our understanding of
asteroids that could impact Earth.

------------------------------------------------------------------------

## OSIRIS-REx Asteroid Sample Collection Overview
...
## Questions from Last Year (2 of 2)

> **Source: Slide 18**

-   In situations like the Osiris Rex touch and go, ...what is a
...
-   Is the asteroid massive enough that the SC is actually orbiting the
    asteroid?
...
the spacecraft body at some radius from the center of mass.
...
Instrument collector is oversized, such that it can internally
...
Arrays will often be single- or dual-axis articulated to minimize solar
...
## Example 3-axis vs. Dual-Spin Stabilization
...
Global Positioning System GD Sentinel M-Code GPS Receiver Mass 2.5 kg
...
-   "Do they carry a surplus of gyros on the ISS now after being close
    to critical loss of the station or is it still not worth the
    weight/space resources?"
...
-   Other mid-size thrusters where the above thrusters aren't sufficient
    (eg, thrust vector control on a large ∆V burn)
...
Graphic from Pioneer 10 spacecraft, history.nasa.gov

-   Spin Rate

    -   Two pairs of coupled thrusters act on opposite sides to change
        spin rate
...
-   Attitude
    ...
    -   Pair of coupled thrusters rotate the spin-axis. Thrusters are
        fired in brief pulses at a precise position in the spacecraft
        rotation. Each pulse moves the axis a few tenths of a degree
        until the desired attitude is reached

------------------------------------------------------------------------

# 7. ACS Architecture, Sizing, and Design
...
requirements, one can generally

estimate a likely architecture based on a mission description and
relative budget.

------------------------------------------------------------------------

## ACS System Design Interpreting a Block Diagram
...
Descent Power and Avionics Module (DPAM) A
...
# 8. Questions and Additional ACS Topics
...
-   SOHO- failed gyro- (coupled with inadequate fault response nearly
    failed the mission)
...
-   Have there been cases where new GN&C technology needed to be
    developed?
...
-   I'd like to know more about the GPS method for attitude
    ...
    -   Cost of deploying a constellation at Mars is prohibitive. If
        ...
        approximately 10 m\^2 just for the GPS power output.

------------------------------------------------------------------------

## Questions from Prior Class (3 of 3)

> **Source: Slide 70**

-   On weights attached to sensors in Chart 23, what missions might
    require sudden, large updates of ATE versus more gradual updates?

    -   Not many, but some would be landers for terrain avoidance
...
-   How was the comet sample in the Deep Impact mission relayed back to
    Earth? Was it just the analysis of the videos and the plume, or was
    there a physical sample that made it back to Earth?

    -   Only data. Instruments (including a spectrometer that can
        analyze the composition of the plume) transmitted data back to
        Earth. The probe relayed its data via the observing spacecraft.

-   How have we still been able to stay in contact with older s/c like
    voyager, and when are we expected to lose any communication with it
...
-   Kinematics
    ...
    -   Describes the motion of points, bodies (objects), and systems of
        bodies (groups of objects) without considering the forces that
        cause them to move.
...
## Example Moment of Inertia of a Ring
...
-   This highly relevant because you can determine the moments of
...
## Example Moment of Inertia of Disk about Edge
...
and incorporate a high-fidelity radar model into the Phoenix EDL
simulation

-   This fixed several anomalies that had the potential to fail the
    mission. But, it

also uncovered new anomalies....
...
-   Deep Impact
    ...
    -   NASA spacecraft designed to study the interior composition of a
        comet by releasing an impactor into the comet

    -   Launched in 2005, it resulted in a successful comet impact
...
However, when only 1 star tracker is in-lock, the whole quaternion from
...
-   Beware of Heritage
    ...
    -   Initially, the team was very skeptical that there was a star
        tracker issue given the significant heritage. Finally, after
        significant convincing, the team investigated and discovered the
        incorrect parameter

Note that the alignment error was found on all three star trackers,
including the

one on the impactor. If it had been limited to the impactor, the issue
may not have been caught until there was insufficient time to fix prior
to the comet flyby.
...
-   **Disturbances:** Major disturbance torques include aerodynamic,
...
-   **Sensors:** Sun sensors, Earth sensors, magnetometers, star
    ...
    provide attitude knowledge with different accuracy, cost, mass,
    power, and complexity.
-   **Control Effectors:** Reaction wheels, momentum wheels, CMGs,
    magnetic torque rods, and reaction-control thrusters provide
    different combinations of accuracy, torque authority, momentum
...
-   **Mission Geometry:** SEP, SPE, phase angle, solar-incidence angle,
    ...
    drive spacecraft pointing architecture.
-   **Redundancy:** ACS commonly uses hardware or functional redundancy
    because loss of attitude control can threaten the entire mission.
-   **Spacecraft Dynamics:** Moment of inertia, angular momentum,
    torque, Euler's moment equations, and Euler angles provide the
    physical basis for modeling spacecraft rotational motion.
-   **Case Studies:** Phoenix demonstrates the importance of simulation
    fidelity and realistic radar testing. Deep Impact demonstrates the
...
Overall, ACS design connects **mission pointing requirements and
spacecraft disturbances to sensors, actuators, control logic, rotational
dynamics, redundancy, and verification**.
